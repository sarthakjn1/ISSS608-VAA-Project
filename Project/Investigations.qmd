...
combined_matches <- bind_rows(
  ...
  #arrange(cargo_id, source, date, desc(total_dwell_time)) %>%
  group_by(cargo_id) %>%
  # Filter to prioritize harbor data
filter(if(any(source == "Harbor")) source == "Harbor" else source =="Transponder")%>%
   #Select the first entry per group
 # slice(1) %>%
  ungroup()  %>%
  rename(
...
```{r}
# this gives 55,563 matches for the 5307 cargos
combined_matches


# there are 1,036 cargos with 1-1 mapping
combined_matches %>%
  group_by(cargo_id) %>%
  summarise(count = n()) %>%
  filter(count ==1) 
...
# there are 2,394  with <=3 mapping
combined_matches %>%
  group_by(cargo_id) %>%
  summarise(count = n()) %>%
  filter(count <=3) 
...
check<- results%>% filter(!is.na(vessel_id))%>%
    distinct(cargo_id)

#print(check)
 
#this gives 2,881 matches for 916 cargos
```
...
combined_results_final <- bind_rows(combined_matches_3, results_valid)
#, combined_matches_fallback)

# Display the combined results
print(combined_results_final)
```
...
#we have reasonable mappings(<=3) for 3095 cargos
combined_results_final %>%
  group_by(cargo_id) %>%
  summarise(count = n()) %>%
  filter(count <=3) 
...
all_cities <- unique(combined_matches$city_of_arrival)
all_vessel_companies <- unique(combined_matches$vessel_company)
...
merged_data <- merge(complete_data, combined_matches, by = c("city_of_arrival", "vessel_company"), all.x = TRUE)
